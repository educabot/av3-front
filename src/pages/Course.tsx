...
export function Course() {
  ...
  const setDocuments = useCoordinationStore((s) => s.setDocuments);
  ...
  const courseDocuments = useMemo(() => {
    ...
    const areaIds = new Set(
      subjects.filter((s) => subjectIds.has(s.id)).map((s) => s.area_id),
    );
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      try {
        setIsLoading(true);
        const res = await coordinationDocumentsApi.list();
        ...
        <div className="space-y-6">
          ...
          <section>
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="body-1-regular text-muted-foreground">
                ({courseCourseSubjects.length})
              </span>
            </div>
            ...
            {courseCourseSubjects.length === 0 ? (
              ...
              <div className="activity-card-bg backdrop-blur-sm border-slate-200 rounded-2xl p-4 space-y-3">
                {courseCourseSubjects.map((cs) => (
                  <div
                    key={cs.id}
                    className="flex items-center justify-between p-4 bg-[#FFFFFF4D] rounded-xl"
                  >
                    <div>
        ...
        <aside>
          ...
          <div className="fill-primary backdrop-blur-sm border-slate-200 rounded-2xl p-6">
            <div className="space-y-4">
              ...
              <InfoRow
                label="MATERIAS"
                value={String(courseCourseSubjects.length)}
              />
              <InfoRow
                label="ÁREAS"
                value={
